...
export default function MasterMenuItemTabbedForm({
  menuItem,
  categories,
  onSave,
  onCancel,
  ...
  const fetchModifiers = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Map master modifier groups to expected format
        const mappedModifiers = modifiersData.map((group: any) => ({
          id: parseInt(group.id) || parseInt(group.modifierGroupId || "0"),
          tblModifierId: parseInt(group.id) || parseInt(group.modifierGroupId || "0"),
  ...
  const fetchPrepZones = async () => {
    try {
      const token = localStorage.getItem("master_admin_token");
      const res = await fetch("/api/master/prep-zone", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
