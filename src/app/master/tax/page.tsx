...
interface Tax {
  ...
  taxCode: string;
  taxname: string;
  taxrate: number | string; // Prisma Decimal type
  createdBy?: string;
  createdDate?: string;
}
...
export default function TaxManagementPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch("/api/master/tax", {
        cache: "no-store",
        headers: {
      ...
      if (response.ok) {
        ...
        toast.error(errorData.error || "Error loading data");
      }
    } catch (error) {
  ...
  const handleSave = async (formData: any) => {
    try {
      ...
      const url = editingTax
        ? `/api/master/tax/${editingTax.tblTaxId}`
        : "/api/master/tax";
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const token = localStorage.getItem("master_admin_token");
      const response = await fetch(`/api/master/tax/${taxToDelete.tblTaxId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      if (response.ok) {
        setTaxes(taxes.filter((tax) => tax.tblTaxId !== taxToDelete.tblTaxId));
  ...
  return (
    <MasterDashboardLayout>
                ...
                <p className="text-2xl font-semibold text-gray-900 dark:text-white">
                  {taxes.length > 0
                    ? formatTaxRate(
                        taxes.reduce(
                          (acc, tax) =>
                            acc + parseFloat(tax.taxrate.toString()),
                          0
                        ) / taxes.length
                      )
                    : "0%"}
        ...
        <div className="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
          ...
          {taxes.length === 0 ? (
            ...
            <DataTable
              ...
              data={taxes}
              keyExtractor={(tax: Tax) => tax.tblTaxId.toString()}
      ...
      <DeleteConfirmationModal
        ...
        title="Delete Tax"
        itemName={taxToDelete?.taxname || ""}
        description={`Are you sure you want to delete the tax "${taxToDelete?.taxname}"? This action cannot be undone and will affect any related menu masters.`}
      />
    </MasterDashboardLayout>
  );
}
