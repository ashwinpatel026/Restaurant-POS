...
export default function ModifiersPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const modifiersRes = await fetch("/api/master/modifier-groups", {
        cache: "no-store",
        headers: {
  ...
  // Navigation handlers
  const handleAdd = () => {
    router.push("/master/modifiers/modifiers/add");
  };

  const handleEdit = (modifier: ModifierGroup) => {
    router.push(`/master/modifiers/modifiers/${modifier.id}/edit`);
  };

  const handleClone = (modifier: ModifierGroup) => {
    router.push(`/master/modifiers/modifiers/add?cloneId=${modifier.id}`);
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      const token = localStorage.getItem("master_admin_token");
      const response = await fetch(`/api/master/modifier-groups/${deletingId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      if (response.ok) {
        setModifiers(modifiers.filter((mod) => mod.id !== deletingId));
    ...
    <MasterDashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <button
            onClick={() => router.push("/master/modifiers")}
