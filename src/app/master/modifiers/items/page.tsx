...
export default function ModifierItemsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [itemsRes, modifiersRes] = await Promise.all([
        fetch("/api/master/modifier-items", {
          headers: {
      ...
      if (itemsRes.ok) {
        ...
        setModifierItems(Array.isArray(itemsData) ? itemsData : []);
      } else {
        console.error(
  ...
  // Navigation handlers
  const handleAdd = () => {
    router.push("/master/modifiers/items/add");
  };

  const handleEdit = (item: ModifierItem) => {
    router.push(`/master/modifiers/items/${item.id}/edit`);
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      const token = localStorage.getItem("master_admin_token");
      const response = await fetch(`/api/master/modifier-items/${deletingId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      if (response.ok) {
        setModifierItems(
          modifierItems.filter((item) => item.id !== deletingId)
        );
