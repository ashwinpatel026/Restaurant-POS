...
export default function MenuCategoriesPage() {
  ...
  // Navigation handlers
  const handleAdd = () => {
    router.push("/master/menu/categories/add");
  };

  const handleEdit = (categoryId: number) => {
    router.push(`/master/menu/categories/${categoryId}/edit`);
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      const token = localStorage.getItem("master_admin_token");
      const response = await fetch(`/api/master/menu-categories/${deletingId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
                          ...
                          <div className="mt-3">
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              ...
                              {category.modifierGroups.length > 6 && (
                                <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400">
                                  +{category.modifierGroups.length - 6} more
                                </span>
                              )}
                            </div>
                          </div>
