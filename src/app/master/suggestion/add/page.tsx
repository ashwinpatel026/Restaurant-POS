...
export default function AddSuggestionPage() {
  ...
  const fetchPrepZones = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setPrepZones(data.filter((pz: PrepZone) => pz.isActive === 1));
      }
    } catch (e) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem("master_admin_token");
      const response = await fetch("/api/master/suggestion", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        toast.error(errorData.error || "Failed to create suggestion");
      }
    } catch (error: any) {
