...
export default function TimeEventsPage() {
  ...
  const fetchEvents = async () => {
    try {
      ...
      if (response.ok) {
        ...
        toast.error(errorData.error || "Error loading events");
      }
    } catch (error) {
  ...
  // Navigation handlers
  const handleAdd = () => {
    router.push("/master/time-event/add");
  };

  const handleEdit = (event: TimeEvent) => {
    router.push(`/master/time-event/${event.id}/edit`);
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      const token = localStorage.getItem("master_admin_token");
      const response = await fetch(`/api/master/time-event/${deletingId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
