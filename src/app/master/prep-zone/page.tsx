  ...
  const handleSave = async (formData: any) => {
    try {
      ...
      const url = editingZone
        ? `/api/master/prep-zone/${editingZone.prepZoneId}`
        : "/api/master/prep-zone";
      ...
      if (response.ok) {
        ...
        try {
          ...
          const errorMessage = errorData.error || "Failed to save prep zone";
          toast.error(errorMessage);
        } catch (jsonError) {
          toast.error("Failed to save prep zone");
        }
      }
    } catch (error: any) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/master/prep-zone/${zoneToDelete.prepZoneId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
