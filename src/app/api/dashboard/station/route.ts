...
function mapStationResponse(station: any) {
  ...
  // Explicitly convert all BigInt fields to avoid serialization errors
  const { tblStationId, updatedBy, ...rest } = station
  return {
    ...rest,
    tblStationId: typeof tblStationId === 'bigint' ? tblStationId.toString() : String(tblStationId || ''),
    updatedBy: updatedBy && typeof updatedBy === 'bigint' ? updatedBy.toString() : (updatedBy || null),
