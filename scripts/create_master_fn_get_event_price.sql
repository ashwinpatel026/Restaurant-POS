-- FUNCTION: public.fn_get_event_price(numeric, text, text[])
-- Master Database Function with menu_master_code support

-- DROP FUNCTION IF EXISTS public.fn_get_event_price(numeric, text, text[]);

CREATE OR REPLACE FUNCTION public.fn_get_event_price(
	p_base_price numeric,
	p_dept_code text DEFAULT NULL,
	p_menu_master_code text[] DEFAULT NULL)
...
BEGIN
    ...
    FROM public."tbl_master_time_events" te
    LEFT JOIN public."tbl_master_menu_master_event" mme 
...
ALTER FUNCTION public.fn_get_event_price(numeric, text, text[])
    OWNER TO postgres;
